...
async function requestMetadata(file: File): Promise<{ pages?: number; size_bytes?: number } | null> {
    ...
    try {
        return await apiFetch<{ pages?: number; size_bytes?: number }>("/api/pdf_tools/metadata", {
            method: "POST",
            body: form,
        });
    } catch {
...
export function StitchPanel() {
    const pluginConfig = usePluginSettings<PluginConfig>("pdf_tools", {});
    const limits = useMemo(
        () => ({
            maxFiles: Math.max(1, Number(pluginConfig.upload?.max_files) || 6),
    ...
    const [planMode, setPlanMode] = useState<"text" | "guided">("guided");
    ...
    const clearQueue = useCallback(() => {
        setUploads((current) => {
            current.forEach(u => URL.revokeObjectURL(u.url));
            return [];
        });
        setInstructions("");
        setGuidedRows([]);
    ...
    const submit = useCallback(
        async (event?: FormEvent) => {
            ...
            try {
                const payload = await withLoader(() =>
                    apiFetch<{ filename: string; pdf_base64: string }>("/api/pdf_tools/stitch", {
                        method: "POST",
                        body: form,
                    }),
                );
                ...
                if (prefs.autoDownload) {
                    ...
                    status.setStatus(`Stitched PDF saved as ${payload.filename}`, "success");
                } else {
        ...
        [uploads, instructions, outputName, prefs.autoDownload, prefs.defaultOutputName, status, withLoader, planMode, guidedRows],
    );
    ...
    return (
        <div className="tool-shell__workspace">
            ...
            <SettingsModal
                ...
                description="Configure default filenames and metadata fetching."
            />
        </div>
    );
}
